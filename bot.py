...
async def safe_edit_text(message, text, reply_markup=None, **kwargs):
    """Edit a Telegram message without crashing when content is unchanged."""
    try:
        return await safe_edit_text(message, text, reply_markup=reply_markup, **kwargs)
...
async def finish_registration(dom, x):
    """Mark Spaceship registration confirmed, then configure Cloudflare.

    A domain can already be registered at Spaceship even if Cloudflare setup
    fails. Never turn a confirmed registration back into registration_pending.
    """
    # Do not rewrite timestamps on every retry. Rewriting the record causes a
    # GitHub commit, which can trigger Railway auto-deploy and create a retry loop.
    if not x.get("registration_confirmed"):
        x["registration_confirmed"] = True
        x["registration_confirmed_at"] = time.time()
    if x.get("registration_status") != "registered":
...
async def pnew(c):
    ...
    await safe_edit_text(c.message, "Project name?")
    await c.answer()
...
async def add(c):
    ...
    await safe_edit_text(c.message, "Existing domains one per line.")
    await c.answer()


@dp.callback_query(F.data == "check")
async def check(c):
    active = pending = error = registered = 0
    ...
    for dom, x in store.data["domains"].items():
        ...
        # If Spaceship registration was already confirmed, do not ask the
        # async registration operation again. Retry only the Cloudflare setup.
        if status == "registered_no_cloudflare" and not x.get("zone_id"):
            before = repr(sorted(x.items()))
            await finish_registration(dom, x)
            status = x.get("status")
            after = repr(sorted(x.items()))
            if after != before:
                changed = True

        elif status in ("registration_pending", "error") and not x.get("zone_id"):
            op_result = None
            op_error = None
            direct_error = None
            direct_checked = False

            # First ask Spaceship for the actual domain record. This is the
            # source of truth: if the domain exists and is registered, the
            # async operation is no longer allowed to keep it stuck pending.
            last_verify = float(x.get("last_spaceship_verify_ts", 0) or 0)
            if time.time() - last_verify >= 60:
                try:
                    info = await spaceship_domain_info(dom)
                    direct_checked = True
                    x["last_spaceship_verify_ts"] = time.time()
                    x["spaceship_lifecycle_status"] = (info or {}).get("lifecycleStatus") if info else None
                    x["spaceship_verification_status"] = (info or {}).get("verificationStatus") if info else None
                    if domain_is_registered(info):
                        await finish_registration(dom, x)
                        status = x.get("status")
                        changed = True
                    elif info is None:
                        direct_error = "Domain not found in Spaceship account yet."
                except Exception as e:
                    direct_checked = True
                    x["last_spaceship_verify_ts"] = time.time()
                    direct_error = str(e)

            # If direct lookup did not confirm registration, check the async
            # operation for pending/failed state and its exact error.
            ...
            # If neither endpoint confirms success, keep it pending. Show the
            # real API error in the record so the user can see what is wrong.
            if status == "registration_pending" and direct_error and op_result is None:
                x["last_error"] = direct_error
            elif status == "registration_pending":
                x["last_error"] = None

        # Once a Cloudflare zone exists, check its real activation status.
        ...
        elif status == "registered_no_cloudflare":
            registered += 1
            err = x.get("cloudflare_error") or x.get("last_error") or "Cloudflare setup failed"
            lines.append(f"🟠 {dom} — Registered at Spaceship\n   Cloudflare setup: {str(err)[:250]}")
        elif status == "registration_pending":
            pending += 1
            lines.append(f"⏳ {dom} — Registration pending")
            if x.get("last_error"):
                lines.append(f"   Check info: {str(x['last_error'])[:250]}")
        elif status == "pending":
            pending += 1
    ...
    lines.extend([
        ...
        f"🟢 Active: {active}",
        f"🟡 Pending: {pending}",
        f"🟠 Registered / CF setup issue: {registered}",
        f"🔴 Error: {error}",
        "",
        "Tap 🔄 Check Status again anytime to refresh.",
    ])
...
async def failed_domains(c):
    ...
    if not failed:
        await safe_edit_text(c.message, "❌ No failed domains.", reply_markup=kb([
            [InlineKeyboardButton(text="⬅️ Domains", callback_data="dmenu")]
        ]))
...
async def retry_purchase(c):
    ...
    try:
        ...
        if not item or item.get("result") != "available":
            ...
            return await safe_edit_text(c.message, 
...
async def retry_cancel(c):
    ...
    await safe_edit_text(c.message, "❌ Retry cancelled", reply_markup=kb([
        [InlineKeyboardButton(text="❌ Failed Domains", callback_data="failed_domains")],
        [InlineKeyboardButton(text="⬅️ Domains", callback_data="dmenu")],
    ]))
    await c.answer()
...
async def retry_confirm(c):
    ...
    try:
        ...
        await safe_edit_text(c.message, f"❌ Retry purchase failed:\n{e}")
    finally:
        sess.pop(c.from_user.id, None)
    await c.answer()
...
async def restore_buy_list_message(c):
    ...
    rows = two_col(buttons)
    rows.append([InlineKeyboardButton(text="✅ Done Buying", callback_data="donebuy")])
    await safe_edit_text(c.message, "Available:", reply_markup=kb(rows))
...
async def done(c):
    sess.pop(c.from_user.id, None)
    await safe_edit_text(c.message, "📊 Purchase session ended. Use 🔄 Check Status after propagation.")
    await c.answer()
...
async def assign(c):
    project = sess.get(c.from_user.id, {}).get("project")
    if not project:
        return await safe_edit_text(c.message, "Open a project first")
    ...
    if not sites or not domains:
        return await safe_edit_text(c.message, "No unassigned HTML or active domains.")
...
async def pick(c):
    ...
    await safe_edit_text(c.message, f"✅ {w['original_filename']} → {dom}")
    ...
    if s["rest"]:
        ...
        sess.pop(c.from_user.id, None)
        await safe_edit_text(c.message, "✅ Assignment complete.")
    await c.answer()
...
async def skip(c):
    ...
    if not s or not s["rest"]:
        sess.pop(c.from_user.id, None)
        await safe_edit_text(c.message, "⏭ Assignment finished.")
        return await c.answer()
    s["site"] = s["rest"].pop(0)
    await show_domains(c.from_user.id, c.message)
    await c.answer()
...
async def ac(c):
    sess.pop(c.from_user.id, None)
    await safe_edit_text(c.message, "❌ Assignment cancelled")
    await c.answer()
        ...
        for dom, d in list(store.data["domains"].items()):
            try:
                if d.get("status") == "registered_no_cloudflare" and not d.get("zone_id"):
                    # Registration is already confirmed. Retry only Cloudflare setup.
                    # IMPORTANT: only persist when something actually changes.
                    # Otherwise a persistent CF error would commit data.json every
                    # cycle -> Railway redeploy -> worker restart -> infinite loop.
                    before = repr(sorted(d.items()))
                    await finish_registration(dom, d)
                    after = repr(sorted(d.items()))
                    if after != before:
                ...
                new_error = str(e)
                if d.get("last_error") != new_error:
                    d["last_error"] = new_error
                    try:
