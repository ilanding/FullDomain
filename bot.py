...
async def confirmcart(c):
    ...
    try:
        ...
        # Submit every cart item one-by-one. The Telegram message is updated
        # before each request so the user can see 1/4, 2/4, etc.
        ...
        msg = f"✅ Purchase requests submitted: {len(submitted)}/{total}"
        if failed:
            msg += f"\n❌ Failed: {len(failed)}"
            for dom, err in failed[:5]:
                msg += f"\n• {dom}: {err[:120]}"
