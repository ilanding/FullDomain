...
async def check(c):
    ...
    for dom, x in store.data["domains"].items():
        ...
        # First check the Spaceship registration operation manually.
        if status in ("registration_pending", "error") and x.get("operation_id") and not x.get("zone_id"):
            # Re-check the Spaceship operation even if a previous background
            # worker marked the domain as error. This lets us recover/show the
            # real Spaceship failure reason instead of "Unknown error".
            op = x.get("operation_id")
            if not op:
                x["status"] = "error"
                x["last_error"] = "No Spaceship operation ID was saved."
                status = "error"
                changed = True
            else:
                try:
                    ...
                    if rs == "success":
                        # Registration is complete: immediately start Cloudflare setup.
                        ...
                        x["status"] = "error"
                        x["last_error"] = r.get("error") or r.get("message") or str(r)
                        status = "error"
                        changed = True
                    else:
            ...
            try:
                if d.get("status") == "registration_pending" and d.get("operation_id"):
                    ...
                    if r.get("status") == "success":
                        ...
                        d["status"] = "error"
                        d["last_error"] = r.get("error") or r.get("message") or str(r)
