...
async def deploy_website(w):
    ...
    # Do not mark the website/deployment live until the public workers.dev URL responds.
    await verify_worker_live(url)
    ...
    return url, "Cloudflare Worker deployed and verified live"
...
async def reconcile_deployments():
    ...
    for wid, w in store.data.get("websites", {}).items():
        ...
        if live:
            continue
        # Only repair the record when the public Worker is actually reachable.
        try:
            await verify_worker_live(w["worker_url"], attempts=2, delay=1)
        except Exception:
            continue
