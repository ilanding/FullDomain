...
async def _cf_email_destination_verified(email):
    # Prefer the exact Cloudflare destination ID saved when the destination
    # was created/found. This avoids relying on account-level list pagination
    # when checking an already-verified address.
    settings = store.data.setdefault("settings", {})
    destination_id = settings.get("email_destination_id")

    if destination_id:
        url = (
            f"https://api.cloudflare.com/client/v4/accounts/"
            f"{cfg.cf_account}/email/routing/addresses/{destination_id}"
        )
        headers = {
        ...
        item = j.get("result") or {}

        # Cloudflare returns the verification timestamp in `verified`.
        # Null/missing means the destination is not verified.
        if str(item.get("email", "")).strip().lower() == email.strip().lower():
            return bool(item.get("verified"))

    # Fallback for older records where the destination ID was not saved.
