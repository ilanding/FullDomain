        ...
        for i in range(0, len(cand), 20):
            ...
            for q in items:
                if q.get("result") == "available":
                    ...
                    # Spaceship's availability API only exposes premium/domain-specific
                    # registration pricing. For ordinary domains it can omit the price.
                    # In that case fetch the current first-year TLD price from Spaceship's
                    # own public TLD pricing page. No hardcoded price is used.
                    if price is None:
