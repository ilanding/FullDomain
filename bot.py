...
async def spaceship_regular_tld_price(domain):
    """Fetch the current public first-year registration price from Spaceship.
    This is only a fallback when /domains/available does not include a regular price.
    No price is hardcoded.
    """
    try:
        tld = domain.rsplit(".", 1)[-1].lower()
        if tld not in {"online", "store", "space", "site"}:
            return None
        ...
        # Prefer the explicit 1-year Register value from the TLD page.
        patterns = [
            r"Register\s+(?:Sale\s+)?(?:<[^>]+>\s*)?\$([0-9]+(?:\.[0-9]{1,2})?)\s*/yr",
            r"Register\s*</[^>]+>\s*(?:<[^>]+>\s*)*\$([0-9]+(?:\.[0-9]{1,2})?)",
            r"Register[^$]{0,500}\$([0-9]+(?:\.[0-9]{1,2})?)\s*/yr",
        ]
        for pat in patterns:
