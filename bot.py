...
async def attach_worker_custom_domain(worker_name, domain, zone_id):
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        # Attach the apex domain and its www hostname to the same Worker.
        # Cloudflare Custom Domains handle TLS/certificates for both hostnames.
        for hostname in (domain, f"www.{domain}"):
            ...
            if r.status_code >= 400:
                raise RuntimeError(
                    f"Cloudflare Worker custom domain {hostname} {r.status_code}: {r.text[:1200]}"
                )
            j = r.json()
            if not j.get("success", True):
                raise RuntimeError(
                    f"Cloudflare Worker custom domain {hostname}: {j.get('errors')}"
                )
            result = j.get("result")
    return result
...
async def _cf_email_catch_all(zone_id, destination):
    """Enable Cloudflare Email Routing catch-all for this zone."""
    ...
    payload = {
        ...
        "matchers": [{"type": "all"}],
        "enabled": True,
        "name": f"Catch-all forwarding to {destination}",
    }
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.put(url, headers=headers, json=payload)
    if r.status_code >= 400:
        raise RuntimeError(f"Cloudflare catch-all {r.status_code}: {r.text[:1000]}")
    j = r.json()
    if not j.get("success"):
        raise RuntimeError(f"Cloudflare catch-all failed: {j.get('errors')}")
...
async def bulk_email_setup(message, pid, prefix, retry_failed_only=False):
    ...
    for dom in active_domains:
        ...
        try:
            ...
            # Enable catch-all forwarding for the whole domain as well as the
            # existing prefix rule. This keeps the current prefix workflow
            # intact while ensuring every other mailbox also reaches the
            # verified destination.
            await _cf_email_catch_all(zone_id, dest)
