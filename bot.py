...
async def pw(c):
    pid = c.data[3:]
    sites = project_sites(pid)
    rows = [[InlineKeyboardButton(text="📤 Upload HTML", callback_data=f"uploadhtml:{pid}")]]
    for w in sites:
        # Website section is only for uploaded HTML files and domain assignment.
        # Live/deployment information is shown in the Deployments section.
        label = f"📄 {w.get('original_filename', 'website.html')}"
        rows.append([InlineKeyboardButton(text=label, callback_data=f"site:{pid}:{w['id']}")])
    rows.append([InlineKeyboardButton(text="⬅️ Project", callback_data=f"viewp:{pid}")])
    await safe_edit_text(c.message, f"💻 Website — {project_title(pid)}\n\n📤 HTML upload karo. Uploaded HTML yahin dikhega; live websites Deployments me hongi.", reply_markup=kb(rows))
    await c.answer()
...
def worker_source_from_html(html_bytes):
    ...
    return (        'const DATA = "' + encoded + '";\n'
        'function html() { const bytes = Uint8Array.from(atob(DATA), c => c.charCodeAt(0)); return new TextDecoder().decode(bytes); }\n'
        'export default { fetch() { return new Response(html(), { status: 200, headers: { "content-type": "text/html; charset=UTF-8", "cache-control": "no-cache" } }); } };\n'
    ).encode("utf-8")
...
async def deploy_worker(script_name, html_path):
    ...
    url = f"https://api.cloudflare.com/client/v4/accounts/{cfg.cf_account}/workers/scripts/{script_name}"
    headers = {"Authorization": f"Bearer {cfg.cf_token}"}
    metadata = {"main_module": "index.js", "compatibility_date": "2026-09-13"}
    files = {
        "metadata": (None, json.dumps(metadata), "application/json"),
        "index.js": ("index.js", source, "application/javascript"),
    }
    async with httpx.AsyncClient(timeout=90) as client:
        r = await client.put(url, headers=headers, files=files)
        if r.status_code >= 400:
...
async def deploy_website(w):
    ...
    return url, "Cloudflare Worker deployed"
...
async def pv(c):
    ...
    if not deps:
        text = "🚀 Deployments\n\nNo live websites yet."
    else:
        lines = ["🚀 Live Websites", ""]
        for d in deps:
            domain = d.get("domain") or "Not assigned"
            lines.append(
                f"🌐 {d.get('worker_name')}\n"
                f"📄 {d.get('original_filename', 'website.html')}\n"
                f"☁️ {d.get('worker_url')}\n"
                f"🔗 Custom: {('https://' + domain) if domain != 'Not assigned' else domain}\n"
...
async def _cf_email_destination_info(email):
    """Return Cloudflare destination-address info for an email, including pre-verified addresses.

    Cloudflare exposes `verified` as a verification timestamp; null means unverified.
    Use the account-level destination-address API and paginate so an older verified
    destination is not missed simply because it is outside the first page.
    """
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for page in range(1, 11):
            r = await client.get(base, headers=headers, params={"page": page, "per_page": 50})
            if r.status_code >= 400:
