...
_regular_tld_price_cache = {}


async def spaceship_regular_tld_price(domain):
    """Get the current regular first-year price from Spaceship's public TLD page.

    The legacy /v1/domains/available API exposes premiumPricing but does not
    expose the normal registration price. This helper therefore reads only
    Spaceship's own public pricing page. No price is hardcoded. If Spaceship
    does not expose a parseable price, return None.
    """
    tld = domain.rsplit('.', 1)[-1].lower()
    if tld not in cfg.tlds:
        return None

    now = time.time()
    cached = _regular_tld_price_cache.get(tld)
    if cached and now - cached[1] < 600:
        return cached[0]

    urls = [
        f"https://www.spaceship.com/domains/gtld/{tld}/",
        f"https://www.spaceship.com/en-GB/domains/gtld/{tld}/",
    ]
    headers = {
    ...
    try:
        async with httpx.AsyncClient(timeout=20, follow_redirects=True, headers=headers) as client:
            for url in urls:
                ...
                # First try structured data embedded by Spaceship.
                structured_patterns = [
                    r'(?is)(?:register|registration|registerSale|registrationPrice|registerPrice)[^{}]{0,1200}?"(?:price|amount|value|registrationPrice|registerPrice)"\s*:\s*"?([0-9]+(?:\.[0-9]{1,2})?)',
                    r'(?is)"(?:registerPrice|registrationPrice)"\s*:\s*"?([0-9]+(?:\.[0-9]{1,2})?)',
                ]
                for pat in structured_patterns:
                    m = re.search(pat, raw)
                    if m:
                        value = float(m.group(1))
                        _regular_tld_price_cache[tld] = (value, now)
                        return value

                # Then inspect visible/embedded text. Remove scripts/styles
                # and HTML tags but keep their textual spacing.
                text = re.sub(r'(?is)<script[^>]*>.*?</script>', ' ', raw)
                text = re.sub(r'(?is)<style[^>]*>.*?</style>', ' ', text)
                text = re.sub(r'(?s)<[^>]+>', ' ', text)
                text = re.sub(r'\s+', ' ', text)

                # Spaceship may show a struck-through list price followed by
                # the sale price, e.g. Register Sale $28.46 $0.98 /yr.
                # Take the last dollar amount in the Register segment.
                m = re.search(r'(?is)Register(?:\s+Sale)?(.*?)(?:/yr|per\s+year)', text)
                if m:
                    amounts = re.findall(r'(?:US)?\$\s*([0-9]+(?:\.[0-9]{1,2})?)', m.group(1))
                    if amounts:
                        value = float(amounts[-1])
                        _regular_tld_price_cache[tld] = (value, now)
                        return value

                # Fallback for pages where /yr is absent from the rendered
                # fragment, but the Register label and price are adjacent.
                m = re.search(r'(?is)Register(?:\s+Sale)?(.{0,2500})', text)
                if m:
                    amounts = re.findall(r'(?:US)?\$\s*([0-9]+(?:\.[0-9]{1,2})?)', m.group(1))
                    if amounts:
                        value = float(amounts[-1])
        ...
        for i in range(0, len(cand), 20):
            ...
            for q in items:
                if q.get("result") == "available":
                        ...
                        for container_key in ("pricing", "prices", "registration", "registrationPricing"):
                            ...
                            if isinstance(container, dict):
                                ...
                                for entry in container:
                                    if not isinstance(entry, dict):
                                        continue
                                    if str(entry.get("operation", "")).lower() == "register":
                                        price = entry.get("price", entry.get("amount", entry.get("value")))
                                        break
                                    # Some Spaceship responses expose regular pricing
                                    # without an operation field.
                                    if price is None:
                                        for key in ("price", "amount", "value", "registrationPrice", "registerPrice"):
                    ...
                    if price is None:
                        price = await spaceship_regular_tld_price(q["domain"])
