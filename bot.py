...
async def start(m):
    if not ok(m):
        return
    await m.answer(
        "🏠 Welcome. Use the Telegram bot menu to access Create Project, My Projects, and My Domains."
    )

@dp.message(Command("create"))
async def create_command(m):
    if not ok(m):
        return
    sess[m.from_user.id] = {"s": "pname"}
    await m.answer("📝 Enter the project name:")


@dp.message(Command("projects"))
async def projects_command(m):
    if not ok(m):
        return
    ...
    if not projects:
        return await m.answer(
            "📁 No projects yet.",
            reply_markup=kb([[InlineKeyboardButton(text="➕ Create Project", callback_data="pnew")]])
        )
    rows = []
    ...
    await m.answer("📁 My Projects", reply_markup=kb(rows))


@dp.message(Command("domains"))
async def domains_command(m):
    if not ok(m):
        return
    rows = []
    ...
    await m.answer("🌐 My Domains", reply_markup=kb(rows))
...
async def text_message(m):
    ...
    if st == "add_domain":
        ...
        dom = (m.text or "").strip().lower().rstrip(".")
        if not re.fullmatch(r"[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.[a-z]{2,63}", dom):
            return await m.answer(
                "❌ Please enter a valid fully qualified domain name.\n\nExample: example.sbs"
            )
        ...
        if existing:
            ...
            if existing_pid == pid:
                return await m.answer(
                    f"⚠️ {dom} is already added to this project.",
                    reply_markup=kb([[InlineKeyboardButton(text="⬅️ Domains", callback_data=f"pd:{pid}")]])
                )
            if existing_pid:
                return await m.answer(
                    f"⚠️ {dom} is already assigned to another project.",
                    reply_markup=kb([[InlineKeyboardButton(text="⬅️ Domains", callback_data=f"pd:{pid}")]])
                )

        await m.answer(f"🔎 Verifying {dom} with Spaceship...")
        try:
            info = await spaceship_domain_info(dom)
            if not domain_is_registered(info):
                return await m.answer(
                    f"❌ {dom} could not be verified as a registered domain in your Spaceship account.\n\nNo changes were made.",
                    reply_markup=kb([[InlineKeyboardButton(text="⬅️ Domains", callback_data=f"pd:{pid}")]])
                )
            ...
            store.data.setdefault("domains", {})[dom] = record

            # Use the same Cloudflare setup path as a completed Spaceship purchase.
            await finish_registration(dom, record)
            await store.save(f"Add existing Spaceship domain: {dom}")

            sess[m.from_user.id] = {"s": "project", "project": pid}
            await m.answer(
                f"✅ Domain verified and added: {dom}\n\n"
                "Cloudflare setup has been started.\n"
                "Use Domains → Check Status to monitor activation.\n"
                "Once active, it will follow the same website and Email Routing workflow as the other project domains.",
                reply_markup=kb([
            ...
            await m.answer(
                f"❌ Domain verification/setup failed:\n{str(e)[:900]}",
                reply_markup=kb([[InlineKeyboardButton(text="⬅️ Domains", callback_data=f"pd:{pid}")]])
            )
        return
    ...
    if st == "buynames":
        ...
        buttons = [InlineKeyboardButton(text=f"{d}\n{p}" if p else f"{d}", callback_data=f"b:{i}") for i, (d, p) in enumerate(out)]
    ...
    if st == "email_destination":
        ...
        if not pid or not p:
            return await m.answer("❌ Project session expired.")
        if "@" not in email:
            return await m.answer("❌ Please enter a valid Gmail or destination email.")
        try:
            ...
            await m.answer(f"❌ Destination creation/verification failed:\n{e}")
        return
    ...
    if st == "email_prefix":
        ...
        if verified:
            await m.answer(
                ...
                "Click “Setup All Project Domains” to continue.",
                reply_markup=kb([
            ...
            await m.answer(
                f"⏳ Email settings saved\n\nDestination: {dest}\nPrefix: {prefix}\n\n"
                "Please verify the destination using the Cloudflare verification email, then click “Check Verification”.",
                reply_markup=kb([
...
async def pd(c):
    ...
    for idx, (d, x) in enumerate(project_domains(pid)):
        status = x.get("status", "unknown")
        icon = "🟢" if status == "active" else "🟡" if status == "pending" else "🟠" if status == "registered_no_cloudflare" else "🔴"
        rows.append([InlineKeyboardButton(text=f"{icon} {d}", callback_data=f"dinfo:{pid}:{idx}")])
...
async def adddomain_start(c):
    ...
    await safe_edit_text(
        c.message,
        "➕ Add an Existing Domain\n\nEnter a domain that is already registered in your Spaceship account.\n\nExample: example.sbs",
        reply_markup=kb([[InlineKeyboardButton(text="⬅️ Domains", callback_data=f"pd:{pid}")]])
    )
    await c.answer()
...
async def dinfo(c):
    try:
        _, pid, idx_text = c.data.split(":", 2)
        idx = int(idx_text)
    except Exception:
        return await c.answer("Invalid domain selection", show_alert=True)

    domains = project_domains(pid)
    if idx < 0 or idx >= len(domains):
        return await c.answer("Domain list has expired. Please refresh Domains.", show_alert=True)
...
async def b(c):
    ...
    if idx < 0 or idx >= len(items):
        ...
        if pid and project(pid):
            await c.answer("This domain list has expired. Refreshing the domain list...", show_alert=True)
            await buy_start(c)
            return
        return await c.answer("This domain list has expired. Please open Buy Domains again.", show_alert=True)
...
async def buyone(c):
    ...
    try:
        ...
        await safe_edit_text(
            c.message,
            f"⏳ Purchasing {dom}...\n\n"
            "Submitting the purchase request to Spaceship."
        )
        ...
        # Mark the item submitted immediately after Spaceship accepts it.
        # Persistence must not block the Telegram purchase flow.
        ...
        # Save after the Telegram state is updated. If GitHub persistence is
        # temporarily slow, the user can still continue with the next domain.
        try:
            await asyncio.wait_for(store.save(f"Domain purchase submitted: {dom}"), timeout=20)
        except Exception as save_error:
            print(f"Purchase persistence warning for {dom}: {save_error}")
...
async def restore_buy_list(c):
    ...
    for i, (d, price) in enumerate(items):
        ...
        label = prefix + d + (f"\n{price}" if price else "")
        buttons.append(InlineKeyboardButton(text=label, callback_data=f"b:{i}"))
    rows = two_col(buttons)
...
async def uploadhtml(c):
    ...
    await safe_edit_text(c.message, "📤 Upload your HTML file(s).\n\nEach uploaded HTML file will be deployed as index.html inside its Worker.")
    await c.answer()
...
async def changehtml(c):
    ...
    await safe_edit_text(c.message, "✏️ Upload the new HTML file.\n\nThis will update the existing Worker/subdomain; no new subdomain will be created.", reply_markup=kb([[InlineKeyboardButton(text="⬅️ Website", callback_data=f"site:{pid}:{wid}")]]))
    await c.answer()
...
async def document_upload(m):
    ...
    if mode not in {"html", "replace_html"}:
        await m.answer("📄 HTML received, but upload mode is not active.\n\nGo to Project → Website → 📤 Upload HTML to upload it.")
        return
    pid = s.get("project")
    if not pid or not project(pid):
        return await m.answer("❌ Project session expired.")
    ...
    try:
        ...
        return await m.answer("❌ Please upload an .html file only.")
    ...
    if mode == "replace_html":
        ...
        await m.answer(f"📄 {filename} received.\n⏳ Updating the HTML on the existing Worker...")
        candidate = dict(w)
    ...
    try:
        ...
        await m.answer(
            f"🚀 Deployment successful!\n\n📄 {filename}\n🌐 {url}\n\n⚠️ workers.dev/HTTPS activation may take a few minutes; this does not indicate a deployment failure.",
            reply_markup=kb([
...
async def emailsetup(c):
    ...
    if not p.get("email_prefix"):
        ...
        return await c.answer()
    sess[c.from_user.id] = {"s": "project", "project": pid}
    await safe_edit_text(c.message, "⏳ Verifying the destination and configuring all active domains in the project...")
...
async def emailverify(c):
    ...
    try:
        ...
        if verified:
            ...
            await safe_edit_text(c.message, f"⏳ {dest} is not verified yet.\n\nOpen the Cloudflare verification email, verify the destination, then click “Check Verification”.", reply_markup=kb([[InlineKeyboardButton(text="🔄 Check Verification", callback_data=f"emailverify:{pid}")], [InlineKeyboardButton(text="⬅️ Email Routing", callback_data=f"pe:{pid}")]]))
    except Exception as e:
...
async def bulk_email_setup(message, pid, prefix, retry_failed_only=False):
    ...
    if not dest or not p:
        await safe_edit_text(message, "❌ Destination or project is missing.", reply_markup=kb([[InlineKeyboardButton(text="⬅️ Project", callback_data=f"viewp:{pid}")]]))
        return 0, 0, 0
    ...
    if not verified:
        await safe_edit_text(
            message,
            f"⏳ {dest} is not verified yet.\n\nVerify it using the verification email, then go to Email Routing → Check Verification.",
...
async def emailretry(c):
    ...
    await safe_edit_text(c.message, "⏳ Retrying failed email routing...")
...
async def main():
    ...
    await bot.set_my_commands([
        ...
        BotCommand(command="create", description="Create Project"),
        BotCommand(command="projects", description="My Projects"),
        BotCommand(command="domains", description="My Domains"),
    ])
