...
async def pw(c):
    ...
    rows = [
        [InlineKeyboardButton(text="📤 HTML", callback_data=f"uploadhtml:{pid}")],
        [InlineKeyboardButton(text="🚀 Deployments", callback_data=f"pv:{pid}")],
...
async def site(c):
    ...
    if w.get("status") == "deployed":
        if not w.get("domain"):
            rows.append([InlineKeyboardButton(text="🌐 Domain", callback_data=f"assignsite:{pid}:{wid}")])
        else:
            text += f"\n\n🌐 Assigned Domain: https://{w['domain']}"
        if w.get("worker_url"):
            text += f"\n☁️ Worker: {w['worker_url']}"
        rows.append([InlineKeyboardButton(text="✏️ HTML", callback_data=f"changehtml:{pid}:{wid}")])
    ...
    rows.append([InlineKeyboardButton(text="⬅️ Back", callback_data=f"pw:{pid}")])
...
async def changehtml(c):
    ...
    await safe_edit_text(c.message, "✏️ New HTML file bhejo.\n\nYe isi Worker/subdomain ko update karega; naya subdomain nahi banega.", reply_markup=kb([[InlineKeyboardButton(text="⬅️ Back", callback_data=f"site:{pid}:{wid}")]]))
    await c.answer()
...
async def document_upload(m):
    ...
    if mode == "replace_html":
        ...
        try:
            ...
            await m.answer(
                ...
                reply_markup=kb([
                    [InlineKeyboardButton(text="🌐 Domain", callback_data=f"assignsite:{pid}:{wid}")],
                    [InlineKeyboardButton(text="🚀 Deployments", callback_data=f"pv:{pid}")],
                    [InlineKeyboardButton(text="⬅️ Project", callback_data=f"viewp:{pid}")],
                ]),
            )
    ...
    try:
        ...
        await m.answer(
            ...
            reply_markup=kb([
                [InlineKeyboardButton(text="🌐 Domain", callback_data=f"assignsite:{pid}:{wid}")],
                [InlineKeyboardButton(text="✏️ HTML", callback_data=f"changehtml:{pid}:{wid}")],
                [InlineKeyboardButton(text="🚀 Deployments", callback_data=f"pv:{pid}")],
                [InlineKeyboardButton(text="⬅️ Project", callback_data=f"viewp:{pid}")],
            ]),
        )
...
async def assignsite(c):
    ...
    try:
        ...
        # Only active, unassigned domains from the same project are shown.
        domains = [
            (d, x) for d, x in project_domains(pid, active_only=True)
            if not x.get("website_id") and x.get("assigned") is not True
        ]
        if not domains:
            return await safe_edit_text(
                c.message,
                "❌ No active unassigned domain available in this project.",
                reply_markup=kb([[InlineKeyboardButton(text="⬅️ Back", callback_data=f"pv:{pid}")]]),
            )

        rows = two_col([
            InlineKeyboardButton(text=d, callback_data=f"pickdomain:{pid}:{wid}:{d}")
            for d, _ in domains
        ])
        rows.append([InlineKeyboardButton(text="⬅️ Back", callback_data=f"pv:{pid}")])
        await safe_edit_text(
            c.message,
        ...
        await safe_edit_text(
            c.message,
            f"❌ Domain list open nahi hua:\n{e}",
            reply_markup=kb([[InlineKeyboardButton(text="⬅️ Deployments", callback_data=f"pv:{pid}")]]) if 'pid' in locals() else None,
        )
...
async def pickdomain(c):
    ...
    try:
        ...
        await safe_edit_text(c.message, f"❌ Domain assign nahi hua:\n{e}", reply_markup=kb([[InlineKeyboardButton(text="⬅️ Back", callback_data=f"pw:{pid}")]]))
    await c.answer()
...
async def pv(c):
    ...
    if not deps:
        ...
        rows = [[InlineKeyboardButton(text="📤 HTML", callback_data=f"uploadhtml:{pid}")], [InlineKeyboardButton(text="⬅️ Project", callback_data=f"viewp:{pid}")]]
    else:
        lines = ["🚀 Deployments", ""]
        rows = []
        seen = set()
        ...
        text = "\n".join(lines).rstrip()
        rows.append([InlineKeyboardButton(text="📤 HTML", callback_data=f"uploadhtml:{pid}")])
        rows.append([InlineKeyboardButton(text="⬅️ Project", callback_data=f"viewp:{pid}")])
    await safe_edit_text(c.message, text, reply_markup=kb(rows))
    await c.answer()
