...
async def text_message(m):
    ...
    if st == "buynames":
        ...
        for i in range(0, len(cand), 20):
            ...
            for q in items:
                if q.get("result") == "available":
                    # Spaceship now returns the standard registration price in
                    # `price` for regular domains and `premiumPricing` for premium
                    # domains. Normalize every supported shape to one numeric value.
                    def _price_value(v):
                        if isinstance(v, (int, float)):
                            return float(v)
                        if isinstance(v, str):
                            try:
                                return float(v.replace("$", "").replace("USD", "").strip())
                            except ValueError:
                                return None
                        if isinstance(v, dict):
                            for k in ("amount", "price", "value", "pricePerYear"):
                                if v.get(k) is not None:
                                    got = _price_value(v.get(k))
                                    if got is not None:
                                        return got
                        return None

                    price = _price_value(q.get("price"))

                    # Some API variants expose the regular price inside a nested
                    # pricing object. Do not invent a price if none is returned.
                    if price is None:
                        for key in ("registrationPrice", "registerPrice", "registration_price", "priceUsd", "amount"):
                            price = _price_value(q.get(key))
                            if price is not None:
                                break

                    # Premium domains: use Spaceship's domain-specific register price.
                    if price is None:
                        ...
                        if isinstance(premium, list):
                            for entry in premium:
                                ...
                                if str(entry.get("operation", "")).lower() == "register" or len(premium) == 1:
                                    price = _price_value(entry.get("price", entry.get("amount", entry.get("value"))))
                                    if price is not None:
                                        break

                    # The public External API currently returns premiumPricing for premium names
                    # but may omit the regular registration price. In that case use the
                    # current Spaceship TLD registration price; never invent a fallback.
                    if price is None:
                        try:
                    ...
                    if price is None:
                        for container_key in ("pricing", "prices", "registration", "registrationPricing"):
                            ...
                            if isinstance(container, dict):
                                price = _price_value(container)
                            elif isinstance(container, list):
                                for entry in container:
                                    if not isinstance(entry, dict):
                                        continue
                                    if str(entry.get("operation", "")).lower() == "register" or not entry.get("operation"):
                                        price = _price_value(entry)
                                        if price is not None:
                                            break
                            if price is not None:
                                break

                    # Final fallback: read the current first-year price from Spaceship's
                    # public TLD page. No hardcoded TLD price is used.
                    if price is None:
        ...
        buttons = [InlineKeyboardButton(text=f"{d}\n${float(p):.2f}" if p is not None else f"{d}", callback_data=f"b:{hashlib.sha1(d.encode()).hexdigest()[:12]}") for i, (d, p) in enumerate(out)]
...
async def show_cart(c):
    ...
    for i in cart:
        d, price = items[i]
        lines.append(f"• {d}" + (f" — ${float(price):.2f}" if price is not None else ""))
        try:
            total += float(price)
        except Exception:
            pass
    ...
    for n, item in enumerate(pending, 1):
        rows.append([
            InlineKeyboardButton(
                text=f"{item['domain']}\n${float(item['price']):.2f}" if item["price"] is not None else f"{item['domain']}",
...
async def restore_buy_list(c):
    ...
    for i, (d, price) in enumerate(items):
        ...
        label = prefix + d + (f"\n${float(price):.2f}" if price is not None else "")
