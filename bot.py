            ...
            for q in items:
                if q.get("result") == "available":
                    ...
                    if price is None:
                        ...
                        # Read fallback pricing dynamically for every TLD allowed in
                        # ALLOWED_PURCHASE_TLDS. Spaceship/API pricing above always wins.
                        # Optional Railway env format:
                        #   TLD_PRICES_ONLINE=...
                        #   TLD_PRICES_STORE=...
                        # etc. (dots are replaced by underscores).
                        env_key = "TLD_PRICE_" + tld.lstrip(".").replace(".", "_").upper()
                        raw_price = os.getenv(env_key)
                        if raw_price:
                            try:
