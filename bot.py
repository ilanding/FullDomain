...
ALLOWED_TLDS = (".online", ".store", ".space", ".site")
...
async def text_message(m):
    ...
    if st == "buynames":
        pid = s.get("project")
        cand = [n + t for n in names(m.text or "") for t in ALLOWED_TLDS]
        out = []
        for i in range(0, len(cand), 20):
            ...
            for q in items:
                if q.get("result") == "available":
                    ...
                    # Regular (non-premium) domains do not always include a
                    # registration price in the legacy availability response.
                    # Resolve the current TLD registration price directly from
                    # Spaceship's public TLD pricing page. No hardcoded prices.
                    if price is None:
                        dom_name = q.get("domain", "")
                        tld = "." + dom_name.rsplit(".", 1)[-1].lower() if "." in dom_name else ""
                        if tld in ALLOWED_TLDS:
                            price = await sp.regular_tld_price(tld)

                    # If Spaceship still cannot provide a price, keep it blank.
        ...
        buttons = [InlineKeyboardButton(text=f"💰 {d}" + (f" — {p}" if p else ""), callback_data=f"b:{i}") for i, (d, p) in enumerate(out)]
...
async def b(c):
    ...
    if idx < 0 or idx >= len(items):
        return await c.answer("Domain no longer available", show_alert=True)
    ...
    for n, item in enumerate(pending, 1):
        rows.append([
            InlineKeyboardButton(
                text=f"💰 {item['domain']}" + (f" — {item['price']}" if item["price"] else ""),
...
async def restore_buy_list(c):
    ...
    for i, (d, price) in enumerate(items):
        ...
        label = prefix + d + (f" — {price}" if price else "")
