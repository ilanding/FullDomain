...
async def confirmcart(c):
    ...
    try:
        ...
        for pos, i in enumerate(cart, 1):
            dom, price = items[i]

            # Live progress in the same Telegram message.
            await safe_edit_text(
                c.message,
                f"🛒 Purchasing domains...\n\n⏳ {pos}/{total} — {dom}"
            )
            ...
            try:
                ...
                # A registration request is considered submitted only when
                # Spaceship returns its async operation ID.
                if not op:
                    raise RuntimeError(
                ...
                submitted.append((dom, op))

                await safe_edit_text(
                    c.message,
                    f"🛒 Purchasing domains...\n\n"
                    f"✅ {pos}/{total} — {dom}\n"
                    f"Request accepted by Spaceship."
                )

            except Exception as e:
                failed.append((dom, str(e)))

                # Show the exact Spaceship error before continuing.
                await safe_edit_text(
                    c.message,
                    f"🛒 Purchasing domains...\n\n"
                    f"❌ {pos}/{total} — {dom}\n"
        ...
        if submitted:
            await store.save("Bulk domain purchase submitted")

        submitted_domains = {dom for dom, _ in submitted}
        s["items"] = [(d, p) for d, p in items if d not in submitted_domains]
        s["cart"] = []
        s["s"] = "buylist"

        lines = [f"🛒 Purchase result: {len(submitted)}/{total} submitted", ""]
        for dom, op in submitted:
            lines.append(f"✅ {dom}")
        ...
        lines.append("\nUse Domains → Check Status for registration progress.")
        ...
        await safe_edit_text(c.message, f"❌ Bulk purchase failed:\n{e}")
