...
async def text_message(m):
    ...
    if st == "buynames":
        ...
        buttons = [InlineKeyboardButton(text=(f"{d}\n{p}" if p else d), callback_data="b:" + d) for d, p in out]
...
async def restore_buy_list(c):
    s = sess.get(c.from_user.id, {})
    rows = [[button] for button in [InlineKeyboardButton(text=(f"{d}\n{p}" if p else d), callback_data="b:" + d) for d, p in s.get("items", [])]]
    rows.append([InlineKeyboardButton(text="✅ Done", callback_data="donebuy")])
...
async def deploy_worker(script_name, html_path):
    ...
    # Cloudflare's current Workers Script Upload API expects multipart/form-data
    # with a metadata part describing the entrypoint. Sending the JavaScript
    # bytes directly as application/javascript can create the Worker resource
    # without reliably installing the uploaded script.
    metadata = {
        "body_part": "worker.js",
    ...
    files = [
        ...
        ("worker.js", ("worker.js", source, "application/javascript")),
    ]

    # The account-level workers.dev subdomain (for this bot: zerothtec.workers.dev)
    # is configured once in Cloudflare. Each Worker automatically gets
    ...
    async with httpx.AsyncClient(timeout=90) as client:
        ...
        if not j.get("success", True):
            raise RuntimeError(f"Cloudflare Worker deploy failed: {j.get('errors')}")
