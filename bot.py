        ...
        for i in cart:
            ...
            try:
                ...
                store.data.setdefault("domains", {})[dom] = {
                    "domain": dom, "registrar": "spaceship", "status": "registration_pending",
                    "operation_id": op, "assigned": False, "project_id": pid, "created_at": time.time(),
                }
                submitted.append(dom)
            except Exception as e:
                failed.append((dom, str(e)))
        if submitted:
            await store.save("Bulk domain purchase submitted")
        submitted_set = set(submitted)
        s["items"] = [(d, p) for d, p in items if d not in submitted_set]
        s["cart"] = []
        s["s"] = "buylist"
        msg = f"✅ {len(submitted)} purchase request(s) submitted."
        if failed:
            msg += f"\n❌ {len(failed)} failed and remain available in the list."
        msg += "\n\nUse Domains → Check Status for registration progress."
        await safe_edit_text(c.message, msg, reply_markup=kb([
            [InlineKeyboardButton(text="🔄 Check Status", callback_data=f"pcheck:{pid}")],
            [InlineKeyboardButton(text="🛒 Buy More", callback_data=f"buy:{pid}")],
        ]))
    except Exception as e:
