...
async def text_message(m):
    ...
    if st == "add_domain":
        ...
        if not valid_domain(dom):
            return await m.answer("❌ Enter a valid domain name, for example: example.sbs")
        ...
        try:
            ...
            if not domain_is_registered(info):
                return await m.answer(f"❌ {dom} could not be verified as a registered domain in the Spaceship account.")

            if not existing:
                existing = {
                    "domain": dom, "registrar": "spaceship", "project_id": pid,
                    "status": "registered_no_cloudflare", "assigned": False,
                    "website_id": None, "registration_confirmed": True,
                    "registration_confirmed_at": time.time(),
                }
                store.data.setdefault("domains", {})[dom] = existing
            else:
                existing["project_id"] = pid
                existing["registrar"] = existing.get("registrar") or "spaceship"
                existing["registration_confirmed"] = True
                existing["registration_confirmed_at"] = existing.get("registration_confirmed_at") or time.time()
            ...
            if existing.get("zone_id"):
                await m.answer(
                    f"✅ {dom} verified successfully.\n\nCloudflare setup has been started. Use Check Status to monitor activation.",
                    reply_markup=kb([
                ...
                await m.answer(
                    f"⚠️ {dom} is verified in Spaceship, but Cloudflare setup could not be completed yet.\n\nError: {existing.get('last_error') or 'Unknown error'}",
                    reply_markup=kb([
                        [InlineKeyboardButton(text="🔄 Check Status", callback_data=f"pcheck:{pid}")],
                        [InlineKeyboardButton(text="⬅️ Domains", callback_data=f"pd:{pid}")],
                    ]),
    ...
    if st == "buynames":
        ...
        buttons = [InlineKeyboardButton(text=f"{d}\n{p}" if p else f"{d}", callback_data=f"b:{i}:{hashlib.sha1(d.encode()).hexdigest()[:8]}") for i, (d, p) in enumerate(out)]
            ...
            if existing:
                ...
                if existing.get("verified"):
                    await m.answer(
                        ...
                        "Ab email prefix bhejo.\n\nExample: info"
                    )
                else:
                    await m.answer(
                        f"📧 Destination set: {email}\n\n"
                        "Cloudflare verification email verify karo.\n\n"
                        "Ab email prefix bhejo.\n\nExample: info"
                    )
                return
    ...
    if st == "email_prefix":
        ...
        if verified:
            await m.answer(
                ...
                "Ab Setup All Project Domains dabao.",
                reply_markup=kb([
            ...
            await m.answer(
                f"⏳ Email settings saved\n\nDestination: {dest}\nPrefix: {prefix}\n\n"
                "Pehle Cloudflare verification email me destination verify karo, phir Check Verification dabao.",
                reply_markup=kb([
...
async def b(c):
    ...
    try:
        if ":" in token:
            idx_text, fingerprint = token.split(":", 1)
            idx = int(idx_text)
        else:
            idx = int(token); fingerprint = ""
    except Exception:
        return await c.answer("Invalid selection", show_alert=True)

    dom = None
    if 0 <= idx < len(items):
        candidate = items[idx][0]
        if not fingerprint or hashlib.sha1(candidate.encode()).hexdigest()[:8] == fingerprint:
            dom = candidate
    if dom is None and fingerprint:
        for candidate, _price in items:
            if hashlib.sha1(candidate.encode()).hexdigest()[:8] == fingerprint:
                dom = candidate
                break
    if not dom:
...
async def show_cart(c):
    ...
    lines.append("\nConfirm karne par cart ke saare domains purchase ke liye submit honge.")
...
async def confirmcart(c):
    ...
    await safe_edit_text(
        c.message,
        "🛒 Cart ready\n\n"
        "Ek-ek karke domain purchase karo.\n"
        "Jis domain par click karoge, usi domain ka purchase request Spaceship ko jayega.",
...
async def buyone(c):
    ...
    try:
        ...
        await safe_edit_text(
            c.message,
            f"⏳ Purchasing {dom}...\n\n"
            f"Spaceship ko request bheji ja rahi hai."
        )
        ...
        await safe_edit_text(
            ...
            f"Domain: {dom}\n"
            f"Operation ID: {op}\n\n"
            f"Baaki domains ke button par click karke ek-ek purchase request bhejo.",
            reply_markup=kb(rows)
        )
...
async def restore_buy_list(c):
    ...
    for i, (d, price) in enumerate(items):
        ...
        label = prefix + d + (f"\n{price}" if price else "")
        buttons.append(InlineKeyboardButton(text=label, callback_data=f"b:{i}:{hashlib.sha1(d.encode()).hexdigest()[:8]}"))
    rows = two_col(buttons)
...
async def document_upload(m):
    ...
    if mode == "replace_html":
        ...
        await m.answer(f"📄 {filename} received.\n⏳ Same Worker par HTML update ho raha hai...")
        candidate = dict(w)
    ...
    await m.answer(f"📄 {filename} received.\n⏳ Cloudflare Worker deploy ho raha hai...")
    try:
...
async def pickdomain(c):
    ...
    if not pid or not wid or not dom:
        return await c.answer("Domain selection expired. Dobara Assign Domain karo.", show_alert=True)
    w = store.data.get("websites", {}).get(wid)
    d = store.data.get("domains", {}).get(dom)
...
async def emailsetup(c):
    ...
    if not p.get("email_prefix"):
        ...
        return await c.answer()
    sess[c.from_user.id] = {"s": "project", "project": pid}
    await safe_edit_text(c.message, "⏳ Verification check karke project ke saare active domains setup ho rahe hain...")
...
async def emailretry(c):
    ...
    if not dest:
        return await c.answer("Destination set nahi hai.", show_alert=True)
    if not prefix:
        return await c.answer("Email prefix set nahi hai.", show_alert=True)

    await c.answer("Failed routing retry ho rahi hai...")
    await safe_edit_text(c.message, "⏳ Failed email routing dobara setup ho rahi hai...")
